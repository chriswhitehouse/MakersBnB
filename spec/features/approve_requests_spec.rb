...
feature 'So I can approve a request' do
  scenario 'I would like to be able to confirm or deny requests on my space' do
    ...
    expect(page).to have_button "Confirm request from #{requester.email}"
    expect(page).to have_button "Deny request from #{requester.email}"
  end
end
